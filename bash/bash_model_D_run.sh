...
#SBATCH --time=12:00:00 ## node time 

## this is a bash script to run the
## R file where B = 100 bootstrapped ML models are run on dataset
## D (raw CBCL items + longitudinal variables + covariates),
## R file: 20_model_D_run_bootstrap_stability.R
## SBATCH options;
## one task, 96 CPUs per task, node time of 12:00:00 
## (all runs exceeding this will be aborted!)

## note: This script will be exectued 100 times in parallel on multiple
## nodes by executing the script bash_model_D_bootstrap.sh via the 
...
## Only the objects that are called in the script 20_model_D_run_bootstrap_stability.R
...
Rscript scripts/20_model_D_run_bootstrap_stability.R $SLURM_ARRAY_TASK_ID 
