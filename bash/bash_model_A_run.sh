...
#SBATCH --time=11:30:00 ## node time 

## this is a bash script to run the
## R file where B = 100 bootstrapped ML models are run on dataset
## A (raw CBCL variables + covariates),
## R file: 09_A_run_bootstrap_stability.R
## SBATCH options;
## one task, 96 CPUs per task, node time of 11:30:00 
## (all runs exceeding this will be aborted!)

## note: This script will be exectued 100 times in parallel on multiple
## nodes by executing the script bash_model_A_bootstrap.sh via the 
...
## loading R module (adapt if other version needed)
...
## Only the objects that are called in the script 09_run_bootstrap_stability.R need to be copied
## relevant files will be copied into directory on SNELLIUS
TMPDIR=${TMPDIR:-/tmp}
...
Rscript scripts/09_A_run_bootstrap_stability.R $SLURM_ARRAY_TASK_ID 
