...
# Date: 2025-09-26
#
# Script Name: 31_A_run_sensitivity_analysis_ML.R
#
# Script Description: This script is supposed to re-run the ML training part for 
# variable set A after removing the top 5% multivariate outliers to 
# see if results are robust. No stability assessment will take place,
# thus, no bootstrapping, models are only trained once
#
#
# Notes: 31_A_run_sensitivity_analysis_ML.R re-runs the analysis
# for variable set A (only raw CBCL scores + covariates)
#
#
...
## IDS that were flagged as multivariate outliers
train_ids_remove <- readRDS(
  here::here("data", "intermediate", "outlierIDs_MCD_dataset_A.rds"))
...
## saving data
filename_dataset_processed <- paste0("prepared_data_A_MCD", iter, ".rds")

## creating directory for preprocessed data if not already existent
if(!dir.exists(here::here("data", "intermediate", "prep_data_MCD"))){
  dir.create(here::here("data", "intermediate", "prep_data_MCD"))
}
...
cat("data preparation finished for model A (MCD outliers removed)", "\n", "\n")
...
if(!dir.exists(
  here::here("data", "intermediate", "workspaces_sensitivity_analysis_MCD"))){
    dir.create(
      here::here("data", "intermediate", "workspaces_sensitivity_analysis_MCD"))
}
# Save the list to an RDS file
filename <- paste0("sensitivity_workspace_model_A_iteration_", iter, ".rds")
saveRDS(
  workspace_objects,
...
cat("duration entire script variable set A, sensitivity analysis: ",
