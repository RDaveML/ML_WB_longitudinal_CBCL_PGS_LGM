...
# Date: 2025-07-24
#
# Script Name: 23_model_E_run_bootstrap_stability.R
#
...
#
#
# Notes: 23_model_E_run_bootstrap_stability.R runs models on variable set E
...
# "bash_model_E_bootstrap" which runs the script "bash_model_E_run" 
# B times
...
if(data_prepared == FALSE){
  ...
  for (iter in iterations) {
    ...
    ## same train and test split as in variable sets B and C
    if (b_iter == 0) {
      train_ids <- readRDS(
    ...
    if(iter == 1){
      saveRDS(data_model_E_base,
              file = here::here("data", "intermediate", "data_model_E.rds"))
    }
    ...
    if(iter == 1){
      data_model_E_train <- data_dummies_E %>%
        filter(FISNumber %in% train_ids)
      
      saveRDS(data_model_E_train,
              here::here("data", "intermediate", "data_model_E_train.rds"))
      
      data_model_E_test <- data_dummies_E %>%
        filter(FISNumber %in% test_ids)
      
      saveRDS(data_model_E_test,
              here::here("data", "intermediate", "data_model_E_test.rds"))
    }
...
if(data_prepared) {
  ...
  iter <- commandArgs(trailingOnly = TRUE) ## for bash script
  iter <- as.numeric(iter)
  if (iter > 1) {
    b_iter <- iter - 1
  } else {
