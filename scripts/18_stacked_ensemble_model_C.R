...
# Date: 2025-06-25
#
# Script Name: 18_stacked_ensemble_model_C.R
#
# Script Description: This script codes the stacked ensemble model 
# for variable set C (CBCL items + PGS + covariates)
...
pred_rf <- readRDS(
  here::here("data", "intermediate", "bootstrap",
             "workspace_model_C_iteration_1.rds"))$run_rf$preds_df_rf 

pred_xgb <- readRDS(
  here::here("data", "intermediate", "bootstrap",
             "workspace_model_C_iteration_1.rds"))$run_xgb$preds_df_xgb 

## loading in full workspace original prediction
workspace_orig <- readRDS(
  here::here("data", "intermediate", "bootstrap",
             "workspace_model_C_iteration_1.rds"))
...
## true Y
data_outcome <- readRDS(here::here(
  "data", "intermediate", "data_outcome_C.RDS"))
...
## not pre-registered, not reported)

## iterate over all bootstrapped workspace items

## creating list of workspaces
stability_metrics <- FALSE ## change to TRUE if this analysis should be done
...
if(stability_metrics) {
  ...
  for (run in 1:length(list_files)) {
    ...
    names(metrics_line)[1] <- "run"
    boot_metrics_rf <- rbind(boot_metrics_rf, metrics_line)
    names(boot_metrics_rf) <- names(metrics_line)
  }
  ...
  ## Calculate and display Bootstrapped CIs of the metrics
  ...
  for (run in 1:length(list_files)) {
    ...
    names(boot_metrics_xgb) <- names(metrics_line)
  }
  ...
  for (run in 1:length(list_files)) {
    ...
    names(boot_metrics_stacked_lm) <- names(metrics_line)
  }
