...
# Date: 2025-10-06
#
# Script Name: 43_E_sensitivity_stacked_ensemble_model_performance.R
#
# Script Description: Re-calculating stacked ensemble model 
# and model performance metrics for variable set E
...
workspace <- readRDS(
  paste0(filepath, "/", "sensitivity_workspace_model_E_iteration_1.rds"))
...
data_outcome  <- readRDS(here::here(
  "data", "intermediate", "data_outcome_E.RDS")) %>%
  filter(FISNumber %in% c(train_ids, test_ids))
...
filename <- "boot_results_performace_E_sensitivity.rds"
