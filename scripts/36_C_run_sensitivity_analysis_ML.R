...
# Date: 2025-09-30
#
# Script Name: 36_C_run_sensitivity_analysis_ML.R
#
# Script Description: This script is supposed to re-run the ML training part for 
# model C after removing the top 5% multivariate outliers to 
# see if results are robust. No stsability assessment will take place
#
#
# Notes: 36_C_run_sensitivity_analysis_ML.R runs model C
# (raw CBCL scores + PGS + covariates)
#
#
...
train_ids_remove <- readRDS(
  here::here("data", "intermediate", "outlierIDs_MCD_dataset_C.rds"))
...
data_model_C_test <- readRDS(
  here::here("data", "intermediate", "data_model_C_test.rds"))

data_model_C <- rbind(data_model_C_train,
                      data_model_C_test)
...
filename <- paste0("sensitivity_workspace_model_C_iteration_", iter, ".rds")
saveRDS(
  workspace_objects,
...
cat("duration entire script (model C, custom functions): ",
