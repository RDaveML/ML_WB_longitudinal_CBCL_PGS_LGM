...
# Date: 2025-07-15
#
# Script Name: 28_inspection_bootstrap_stability_model_E.R
#
# Script Description: 
# This script does the bootstrapping stability check of ML predictions 
## (Riley et. 2023) for the models estimated on variable set E
## (CBCL items + longitudinal variables + PGS + covariates). 
...
if(outcome_saved == FALSE){
  ...
  ## have PGS
  full_prepared_data_E <- readRDS(
    here::here(
      "data", "intermediate", "prep_data_E", "full_prepared_data_E_1.rds"))
  data_full_raw <- rbind(full_prepared_data_E$x_train,
                         full_prepared_data_E$x_test)
  ...
  saveRDS(data_true_y, here::here("data", "intermediate", "data_outcome_E.RDS"))
  rm(data_full_raw)
  rm(full_prepared_data_E)
}
...
ggsave(filename = "E_cal_inst_smooth_rf.png",
       plot = all_plots_models$rf$plot_cal_inst_model,
       device = "png",
       path = here::here("data", "intermediate", "bootstrap", "plots"),
       create.dir = TRUE)

ggsave(filename = "E_cal_inst_smooth_xgb.png",
...
# Save the plot
png(filename = file.path(plot_path, "E_mape_inst_rf.png"),
    width = 800, height = 600)
replayPlot(all_plots_models$rf$plot_mape_inst_model)
dev.off()

png(filename = file.path(plot_path, "E_mape_inst_xgb.png"),
    width = 800, height = 600)
...
save.image(
  here::here("data", "intermediate", "workspace_stability_model_E.RData")
)
## eoS
