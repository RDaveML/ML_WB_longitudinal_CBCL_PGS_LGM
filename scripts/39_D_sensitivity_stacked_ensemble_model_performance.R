...
# Date: 2025-10-06
#
# Script Name: 42_D_sensitivity_stacked_ensemble_model_performance.R
#
# Script Description: Re-calculating stacked ensemble model 
# and model performance metrics for variable set D 
# (CBCL items + longitudinal variables) after removing 5% MCD outliers
...
workspace <- readRDS(
  paste0(filepath, "/", "sensitivity_workspace_model_D_iteration_1.rds"))


## 1) Loading in train test split ids
...
data_outcome  <- readRDS(here::here(
  "data", "intermediate", "data_outcome_D.RDS")) %>%
  filter(FISNumber %in% c(train_ids, test_ids))
...
## coding a linear regression machine learning model
...
filename <- "boot_results_performace_D_sensitivity.rds"
