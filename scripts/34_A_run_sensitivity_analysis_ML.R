...
# Date: 2025-09-26
#
# Script Name: 34_A_run_sensitivity_analysis_ML.R
#
# Script Description: This script is supposed to re-run the ML training part for 
# model A after removing the top 5% multivariate outliers to 
# see if results are robust. No stsability assessment will take place
#
#
# Notes: 34_A_run_sensitivity_analysis_ML.R runs model A (only raw CBCL scores + covariates)
#
#
...
if(loading_again){
  ...
  filepath_prep_data <- here::here("data", "intermediate", "prep_data_A")
  ...
  ## loading in training indices to remove
  indices_remove <- here::here(
    "data", "intermediate", "outlierIDs_MCD_dataset_A.rds")
  
  x_train <- x_train %>%
    filter(!FISNumber %in% indices_remove)
