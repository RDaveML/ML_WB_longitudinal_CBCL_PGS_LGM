...
if(outcome_saved == FALSE){
  ...
  ## note: model E contains only individuals with genetic data
  train_ids <- readRDS(
...
# Save the plot
png(filename = file.path(plot_path, "E_mape_inst_rf.png"), width = 800, height = 600)
replayPlot(all_plots_models$rf$plot_mape_inst_model)
dev.off()

png(filename = file.path(plot_path, "E_mape_inst_xgb.png"), width = 800, height = 600)
