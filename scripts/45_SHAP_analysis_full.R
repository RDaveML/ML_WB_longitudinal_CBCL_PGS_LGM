...
# Date: 2025-10-10
#
# Script Name: 45_SHAP_analysis_full.R
#
# Script Description:
# Combining the SHAP analysis for the original models from 
# scripts 12, 17, 21, 26 and 30, without the bootstrapping
# To have all calculations and plots in one script
#
#
# Notes:
#
...
if(files_final == FALSE){
  ...
  prep_data_folders <- paste0(filepath_intermediate, "/",
                              grep("MCD",
                              grep("prep_data",
                                  list.files(here::here("data", "intermediate")),
                                  value = TRUE),
                              value = TRUE,
                              invert = TRUE))
  
  ## CONTINUE HERE!! Next step is to save filenames of first iteration of 
  ## prep data and copying them over
  ...
  for(file in 1:length(filenames_prep_data)){
    file.copy(from = paste0(prep_data_folders[file], "/", filenames_prep_data[file]), 
...
names(SHAP_analysis_re_run) <- paste0("SHAP_analysis_OG_",
                                      feature_sets)
...
output_full_SHAP_plotting <- capture.output({
  shap_plots_full <<- lapply(feature_sets, function(x){ ## assign to global env!
    ...
    alg_compact <- lapply(alg_objects, function(ob){  ## initiating algorithm lapply statement
      ...
      filename_global_plot <- paste0("global_SHAP_", alg_name, "_", x, "_OG.png")
...
## extracting all global plots in one list

## Repeat this once updated results (only top 20) are in 
...
ggsave(filename = "beeswarm_plot_A.pdf",
...
ggsave(filename = "beeswarm_plot_B.pdf",
...
ggsave(filename = "beeswarm_plot_C.pdf",
...
ggsave(filename = "beeswarm_plot_D.pdf",
...
ggsave(filename = "beeswarm_plot_E.pdf",
...
saveRDS(shap_plots_full, file = here::here(
  "data", "final", "shap_plots_full_OG.rds"))

## re-loading data to not run entire script again
# shap_plots_full <- readRDS(here::here("data", "final", "shap_plots_full_OG.rds"))
