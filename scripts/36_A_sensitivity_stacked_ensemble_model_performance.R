...
# Date: 2025-10-06
#
# Script Name: 36_A_sensitivity_stacked_ensemble_model_performance.R
#
# Script Description: Re-calculating stacked ensemble model 
# and model performance metrics for variable set A
## (CBCL items + covariates) after removing 5% MCD outliers
# (sensitivity analysis)
#
#
# Notes: Only the random forest model and the xgb model were included because 
# the svr model did not produce stable results in original analysis 
# on variable set A
#
#
...
## loading in workspace from sensitivity analysis model training
filepath <- here::here(
...
data_outcome <- readRDS(here::here(
  "data", "intermediate", "data_outcome_A.RDS")) %>%
    filter(FISNumber %in% train_ids | FISNumber %in% test_ids)
