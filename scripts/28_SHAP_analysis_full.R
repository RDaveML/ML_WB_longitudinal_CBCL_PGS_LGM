...
# Date: 2025-10-10
#
# Script Name: 28_SHAP_analysis_full.R
#
# Script Description:
# This script calculates
# Shapley Additive exPlanation (SHAP) values for the 
# all xgboost and random forest models estimated on all variables sets (A-E)
# 
#
#
# Notes: Only original models (concluded to be stable) are used,
# no bootstrapped models
...
#
# Note: This script was run on a computational server (ntr-compute1) 
# with parallelizing SHAP value calculation over 48 cores
# 
...
## copying files to "final" folder (if not yet done)
files_final <- TRUE ## change to FALSE if data not yet at right folder

if(files_final == FALSE){
  
  filepath_intermediate <- here::here("data", "intermediate")
  
  filepath_bootstrap <- paste0(filepath_intermediate, "/bootstrap")
  
  files_workspaces_OG <- grep("iteration_1.rds",
                              list.files(filepath_bootstrap),
                              value = TRUE)
  
  list_workspaces_OG <- paste0(filepath_bootstrap, "/",
                               grep("iteration_1.rds",
                                    list.files(filepath_bootstrap),
                                    value = TRUE)
                               )
  
  ## copying files over to final folder
  if(!dir.exists(here::here("data", "final", "OG_workspaces_1"))){
    dir.create(here::here("data", "final", "OG_workspaces_1"))
  }
  
  dir_final <- here::here("data", "final", "OG_workspaces_1")
  
  for(file in 1:length(list_workspaces_OG)){
    file.copy(from = list_workspaces_OG[file], 
              to = paste0(dir_final, "/", files_workspaces_OG[file]))
  }
  
  ## copying prepared data over (excluding MCD data)
  prep_data_folders <- paste0(filepath_intermediate, "/",
                              grep("MCD",
                              grep("prep_data",
  ...
  if(!dir.exists(here::here("data", "final", "OG_prep_data"))){
    dir.create(here::here("data", "final", "OG_prep_data"))
  }
  dir_final_prep_data <- here::here("data", "final", "OG_prep_data")
  
  filenames_prep_data <- vector()
  
  for(folder in 1:length(prep_data_folders)){
    filenames_prep_data[folder] <- 
      grep("_1.rds", list.files(prep_data_folders[folder]), value = TRUE)
  }
  
  for(file in 1:length(filenames_prep_data)){
    file.copy(from = paste0(prep_data_folders[file], "/",
                            filenames_prep_data[file]), 
              to = paste0(dir_final_prep_data, "/", filenames_prep_data[file]))
  }
...
SHAP_analysis_OG <- lapply(feature_sets, function(x){
...
names(SHAP_analysis_OG) <- paste0("SHAP_analysis_OG_",
                                      feature_sets)

saveRDS(SHAP_analysis_OG, file = here::here(
  "data", "final", "workspace_SHAP_analysis_OG.rds"))
...
SHAP_analysis_OG <- readRDS(here::here(
  "data", "final", "workspace_SHAP_analysis_OG.rds"))
...
output_full_SHAP_plotting <- capture.output({
  shap_plots_full <<- lapply(feature_sets, function(x){ 
    ...
    feature_set_looped <- grep(paste0("_", x), names(SHAP_analysis_OG))
    
    ## workspace object of the current feature set
    ws_looped <- SHAP_analysis_OG[[feature_set_looped]]
    ...
    ## initiating algorithm lapply statement
      ...
      filename_bee_plot <- paste0("bee_plot_", alg_name, "_", x, "_OG.png")
...
if(!dir.exists(here::here("data", "plots", "OG"))){
  dir.create(here::here("data", "plots", "OG"))
}

writeLines(output_full_SHAP_plotting,
           here::here("data", "plots", "OG",
                      "output_SHAP_plotting_OG.txt"))
...
## customized)
saveRDS(shap_plots_full, here::here("data", "final", "shap_plots_full_OG.rds"))
...
# shap_plots_full <- 
## readRDS(here::here("data", "final", "shap_plots_full_OG.rds"))
...
for(i_df in 1:length(importance_dfs)){
  ...
  filename <- paste0("SHAP_importance_df_", alg, "_", set, ".rds")
...
if(!dir.exists(here::here("data", "plots", "SHAP"))){
  dir.create(here::here("data", "plots", "SHAP"))
}

ggsave(filename = "beeswarm_plot_A.png",
...
ggsave(filename = "beeswarm_plot_B.png",
...
ggsave(filename = "beeswarm_plot_C.png",
...
ggsave(filename = "beeswarm_plot_D.png",
...
ggsave(filename = "beeswarm_plot_E.png",
...
cat("duration plotting complete SHAP analysis (original analysis): ",
