...
# Date: 2025-09-30
#
# Script Name: 38_E_run_sensitivity_analysis_ML.R
#
# Script Description: This script is supposed to re-run the ML training part for 
# variable set E after removing the top 5% multivariate outliers to 
# see if results are robust. No stability assessment will take place,
# thus, no bootstrapping, models are only trained once
#
#
#
# Notes: 38_E_run_sensitivity_analysis_ML.R re-runs the analysis
# for variable set E
# (raw CBCL scores + longitudinal variables + PGS + covariates)
#
#
...
train_ids_remove <- readRDS(
  here::here("data", "intermediate", "outlierIDs_MCD_dataset_E.rds"))
...
data_model_E_test <- readRDS(
  here::here("data", "intermediate", "data_model_E_test.rds"))

data_model_E <- rbind(data_model_E_train,
                      data_model_E_test)
...
filename <- paste0("sensitivity_workspace_model_E_iteration_", iter, ".rds")
saveRDS(
  workspace_objects,
...
cat("duration entire script variable set E, sensitivity analysis: ",
