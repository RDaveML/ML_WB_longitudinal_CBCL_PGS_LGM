...
## (Riley et. 2023) for the models estimated on variable set B
## (PGS + covariates). Assessing if model predictions are stable and if feature
...
if(outcome_saved == FALSE){
  ...
  data_full_raw <- rbind(full_prepared_data_B$x_train,
                         full_prepared_data_B$x_test)
...
data_true_y <- readRDS(
  here::here("data", "intermediate", "data_outcome_B.RDS")) %>%
...
# Save the plot
png(filename = file.path(plot_path, "B_mape_inst_rf.png"),
    width = 800, height = 600)
replayPlot(all_plots_models$rf$plot_mape_inst_model)
dev.off()

png(filename = file.path(plot_path, "B_mape_inst_xgb.png"),
    width = 800, height = 600)
