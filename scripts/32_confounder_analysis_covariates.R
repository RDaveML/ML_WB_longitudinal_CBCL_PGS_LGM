...
# Date: 2025-09-30
#
# Script Name: xx_linear_model_predictions_covariates.R
#
# Script Description: This script contains a pre-registered sensitivity analysis
# the predictions from all models on the traing set are regressed on 
...
filepath_scistor <- here::here("data", "intermediate", "bootstrap")
...
full_models_list <- lapply(feature_sets, function(set){
  ...
  filename_workspace <- grep(paste0("workspace_model_", set),
                             list.files(filepath_scistor), value = TRUE)[1]
  
  workspace <- readRDS(paste0(filepath_scistor,
                              "/", 
                              filename_workspace))
  ...
  list_set <- lapply(algorithm_type, function(alg){
    ...
    ## algorithm iterated
    pred_df <- 
      workspace[[grep(alg, names(workspace))]][[grep(paste0("preds_df_", alg),
    ...
    p <- ggplot(t_small, aes(y=res, x=conf, fill=r2_type)) +
      ...
      labs(title = paste0("Feature set: ", set, "\n", "Algorithm: ", alg))
    
    
    
    ## CONTINUE HERE!!! Run this over all models! Check in the confounder 
...
combined_plot <- cowplot::plot_grid(plotlist = decomposition_plots, ncol = 2, nrow = 5)
combined_plot


## Are results the same when removing the MCD outliers first? 

## CONTINUE HERE!!!
## replicate with the test set based on the workspaces of the re-ran models
## with MCD outliers removed (still running as of 03-10)
