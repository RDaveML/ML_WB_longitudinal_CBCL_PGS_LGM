...
#
#
# Script Name: 02_data_exploration_compressed.R
# 
# Script Description: 
#
...
# 
#
# Notes: Most descriptive steps were removed, illustrative exploration steps
# and extraction and saving of intermediate objects needed for later
# analysis left in this script
#
...
## CBCL / YSR variables
## table on gitHub in the doc directory
if(!dir.exists(here::here("doc"))){
  dir.create(here::here("doc"))
}
CBCL_YSR_items <- read_excel(
  here::here("doc", "Table_S1_Pre-reg_table_CBCL_features_GMM.xlsx"),
                             col_names = FALSE)[-1,]
...
old_items <- c("q51om3", "q79om3", # YS3
               ...
               "q2om10", "q4om10", "q5om10", "q28om10", "q108om10", "q99om10",
               # YS10)
               "q2om12", "q4om12", "q5om12", "q28om12", "q128om12", "q99om12",
               # YS12)
...
ea_vars <- sort(c("ea4_agg", "ea4_age_agg", "ea4_info_agg", "ea4_n_agg", 
                  # subject
...
saveRDS(
  list(CBCL_YSR_items_vec, CBCL_items_vec, YSR_items_vec, ea_vars, qol_vars),
...
# Process each line
for (line in lines) {
  # extract the variable name and labels
...
## filtering out participants with no outcome, then calculate how
...
for(col in 2:ncol(data_age_vars_twins)){
  cat(
    "frequencies variable", "'", colnames(data_age_vars_twins)[col], "'", "\n",
...
## Non-twin participants will be discarded because they did not participate
## in the first 5 YNTR surveys, insufficient data for longitudinal modeling!

## Thus outliers in age of participation no concern
#------------------------------------------------------------------------------
...
## issue: all CBCL features in the current dataset have more than 
## 50% missing! 
...
for(col in 3:ncol(data_age_vars_final)){
  cat(
    "frequencies variable", "'", colnames(data_age_vars_final)[col], "'", "\n",
...
## includes participants with at least one QoL measure and at least one YNTR
## participation
...
pdf(here::here("data", "intermediate", "histograms_CBCL.pdf"))
...
pdf(here::here("data", "intermediate", "histograms_spybs.pdf"))
...
saveRDS(labels_table, here::here("data", "intermediate", "labels_table.rds"))

# list of items per CBCL question
saveRDS(CBCL_questions_list, here::here("data", "intermediate",
                                        "CBCL_questions_list.rds"))
