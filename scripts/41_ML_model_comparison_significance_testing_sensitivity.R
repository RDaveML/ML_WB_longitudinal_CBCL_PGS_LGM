...
# Date: 2025-08-21
#
# Script Name: 41_ML_model_comparison_significance_testing_sensitivity.R
#
...
# performs best. (sensitivity analysis! Models were trained on datasets with
# top 5% MCD outliers discarded)
#
#
...
boot_performance_A <- readRDS(here::here(
  ...
  "boot_results_performace_A_sensitivity.rds"))
...
boot_performance_B <- readRDS(here::here(
  "data", "intermediate", "bootstrap",
  "boot_results_performace_B_sensitivity.rds"))
...
boot_performance_C <- readRDS(here::here(
  "data", "intermediate", "bootstrap",
  "boot_results_performace_C_sensitivity.rds"))
...
boot_performance_D <- readRDS(here::here(
  "data", "intermediate", "bootstrap",
  "boot_results_performace_D_sensitivity.rds"))
...
boot_performance_E <- readRDS(here::here(
  "data", "intermediate", "bootstrap",
  "boot_results_performace_E_sensitivity.rds"))
...
list_model_metric_dfs <- lapply(1:length(list_performances), function(x){
  ...
  for(mod in 1:length(list_performances[[x]])){
    metrics <- c(
      feature_set = feature_set, unlist(list_performances[[x]][[mod]]))
...
model_performance_df <- do.call(rbind, list_model_metric_dfs) %>%
  ...
  mutate(across(contains("rmse") | contains("r2") | contains("mae"),
                as.character)) %>%
  mutate(across(contains("rmse") | contains("r2") | contains("mae"),
                as.numeric)) %>%
...
saveRDS(model_performance_df, file = here::here(
  "data", "final", "model_performance_df_sensitivity.rds"))
...
plot_RMSE <- 
  ...
  labs(subtitle = "Metric: RMSE, error bars symbolize bootstrapped 95% 
  ...
  theme(plot.title = element_text(size=15, face="bold.italic"),
        ...
        axis.title.x = element_text(
          face="bold", size=15, angle=0, vjust = -1.2),
